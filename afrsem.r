...
files <- file.path('.', paste(substring(samples$sample, 1,4),".isoforms.results", sep = ""))
names(files) <- samples$sample ##得到'.sf文件路径

txi <- tximport(files, type="rsem", tx2gene=tx2gene,countsFromAbundance = c("lengthScaledTPM"))##tximport导入结果
